...
if (-not (Test-Path $PackageDirectory)) {
    throw "Windows Native package directory not found: $PackageDirectory. Run Package-WindowsNative.ps1 first."
}
...
$issPath = Join-Path $PSScriptRoot "StadiaX-WindowsNative.iss"
...
foreach ($relativePath in $requiredFiles) {
    ...
    if (-not (Test-Path $path)) {
        throw "Windows Native installer source is incomplete. Missing: $relativePath"
    }
}
...
if (-not (Test-Path $setupPath)) {
    throw "Windows Native installer was not created: $setupPath"
}
...
Write-Host "Windows Native installer: $setupPath"
