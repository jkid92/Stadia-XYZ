...
internal sealed class UiLocalization
{
    ...
    private static readonly (string English, string Italian)[] Prefixes =
    {
        ("Version ", "Versione "),
        ("Battery: ", "Batteria: "),
        ("Selected: ", "Selezione: "),
    ...
    internal void SetLanguage(string languageCode)
    {
        ...
        if (LanguageCode.Equals(normalized, StringComparison.OrdinalIgnoreCase))
        {
            return;
        }
    ...
    internal string Translate(string? text)
    {
        ...
        if (Italian.TryGetValue(text, out var translated))
        {
            return translated;
        }
        ...
        {
            if (text.StartsWith(prefix.English, StringComparison.Ordinal))
            {
                return prefix.Italian + text[prefix.English.Length..];
            }
        }
    ...
    private void ApplyControl(Control control)
    {
        if (ShouldTranslateText(control))
        {
            ...
            if (control is not TextBox && _attachedControls.Add(control))
            {
                control.TextChanged += TranslateChangedControl;
            }
        }
    ...
    private void TranslateChangedControl(object? sender, EventArgs e)
    {
        ...
        if (translated.Equals(control.Text, StringComparison.Ordinal))
        {
            return;
        }
    ...
    {
        return control is Form or Label or Button or CheckBox or RadioButton or GroupBox or TabPage or ModernTabButton ||
               control is TextBox { ReadOnly: true } textBox && textBox.TextLength < 400 && !textBox.Text.Contains('|');
    }
    ...
    private static string LoadLanguageCode()
    {
        ...
        if (previewLanguage is "it" or "en")
        {
        ...
        return CultureInfo.CurrentUICulture.TwoLetterISOLanguageName.Equals("it", StringComparison.OrdinalIgnoreCase)
            ? "it"
            : "en";
    }
    ...
    private static void SaveLanguageCode(string languageCode)
    {
        try
        {
            ...
            File.WriteAllText(temporaryPath, JsonSerializer.Serialize(new { language = languageCode }, new JsonSerializerOptions { WriteIndented = true }));
