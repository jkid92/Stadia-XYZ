...
.\build\Package-WindowsNative.ps1 -Version v0.5.20.5
.\build\Build-WindowsNativeInstaller.ps1 -Version v0.5.20.5
```

This writes `Stadia-X-Windows-Native-v0.5.20.5.zip` and `Stadia-X-Windows-Native-v0.5.20.5-Setup.exe` under `dist/`.

Pushing to `windows-native-experiment` also builds the Windows Native setup as a GitHub Actions artifact. Pushing a tag like `windows-native-v0.5.20.5` creates a prerelease with the ZIP, setup EXE, and SHA256 files attached.
